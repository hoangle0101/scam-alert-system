...
export function AdminDashboard() {
  ...
  const fetchDashboardData = async (showRefreshAnimation = false) => {
    ...
    try {
      ...
      setPendingReports(
        reportsData.filter((r: any) => r.status === "pending").slice(0, 5),
      );
  ...
  const exportToCSV = () => {
    ...
    a.setAttribute(
      "download",
      `scam_guardian_threat_export_${new Date().toISOString().slice(0, 10)}.csv`,
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Real-time overview of the AI defense grid and system metrics.
          </p>
        </div>
        ...
        {[
          {
            ...
            sub: `${stats?.scans_today || 0} in the last 24h`,
            icon: FileSearch,
          ...
          },
          {
            ...
            sub: `${stats?.active_users || 0} currently active`,
            icon: Users,
            color: "text-blue-500",
          },
          {
          ...
          <Card
            key={i}
            className="bg-dark-800 border-dark-600 relative overflow-hidden group"
          >
            ...
            <CardContent className="p-6 relative z-10">
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`p-2 rounded-lg bg-dark-900 border border-dark-700 ${stat.color} shadow-lg shadow-black/50`}
                >
                  <stat.icon size={20} />
                </div>
          ...
          <CardHeader className="flex flex-row items-center justify-between border-b border-dark-700 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1">
                Geographic distribution of detected cyber threats
              </p>
            </div>
          </CardHeader>
            ...
            <CardContent className="p-5 space-y-5">
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <div className="text-sm font-mono text-white bg-dark-900 px-2 py-1 rounded border border-dark-700">
              ...
              <div>
                <div className="flex justify-between text-[10px] text-slate-400 font-bold uppercase mb-2">
                  ...
                  <span className="text-green-400">
                    {metrics?.avg_processing_time_ms || 0} ms
                  </span>
                </div>
                ...
                <p className="text-[9px] text-slate-600 mt-1">
                  Excellent performance. Target &lt; 50ms.
                </p>
              </div>

              <div className="pt-4 border-t border-dark-700">
                <div className="text-[10px] text-slate-400 font-bold uppercase mb-3">
                  Threat Distribution
                </div>
                <div className="space-y-3">
                  {[
                    {
                      label: "Phishing URLs",
                      value: metrics?.phishing_detected || 0,
                      color: "bg-red-500",
                    },
                    {
                      label: "Suspicious Content",
                      value: metrics?.suspicious_count || 0,
                      color: "bg-yellow-500",
                    },
                    {
                      label: "Legitimate",
                      value: metrics?.legitimate_count || 0,
                      color: "bg-green-500",
                    },
                  ].map((item) => (
                    <div
        ...
        <Card className="lg:col-span-2 bg-dark-800 border-dark-600">
          <CardHeader className="flex flex-row items-center justify-between border-b border-dark-700 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1">
                Live stream of AI scan results
              </p>
            </div>
            <Button
              onClick={() => navigate("/admin/threat-intel")}
              variant="outline"
              size="sm"
              className="border-dark-700 text-slate-400 hover:text-white"
            >
              View All Scans
            </Button>
          </CardHeader>
                    ...
                    stats.recent_scans.map((scan: any) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="text-xs font-bold text-slate-200 truncate max-w-[250px] font-mono">
                            {scan.input_value}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`text-[9px] font-bold px-2 py-1 rounded border shadow-sm ${
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="flex-1 h-1 w-12 bg-dark-900 rounded-full overflow-hidden">
                              <div
                                className={`h-full ${scan.confidence > 0.8 ? "bg-red-500" : "bg-brand-500"}`}
          ...
          <CardHeader className="flex flex-row items-center justify-between border-b border-dark-700 pb-4">
            <div>
              <CardTitle className="text-sm font-bold uppercase tracking-widest text-slate-400 flex items-center gap-2">
                <AlertTriangle size={16} className="text-yellow-500" /> Action
                Required
              </CardTitle>
            </div>
            <Button
              ...
              variant="outline"
              size="sm"
              className="border-dark-700 text-slate-400 h-8 px-2"
            >
              Review Queue
            </Button>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-dark-700/50">
              {loading ? (
                ...
                pendingReports.map((report: any, idx) => (
                  <div
                    key={idx}
                    className="p-4 hover:bg-dark-900/50 transition-colors cursor-pointer"
                    onClick={() => navigate("/admin/reports")}
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span
                        className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${report.type === "scam_report" ? "bg-red-500/10 text-red-400" : "bg-blue-500/10 text-blue-400"}`}
                      >
                        {report.type === "scam_report"
                          ? "Scam Report"
                          : "Appeal"}
                      </span>
            ...
            {pendingReports.length > 0 && (
              <div className="p-3 border-t border-dark-700 text-center">
                <button
                  onClick={() => navigate("/admin/reports")}
                  className="text-[10px] font-bold text-brand-400 hover:text-brand-300 uppercase tracking-widest transition-colors"
                >
