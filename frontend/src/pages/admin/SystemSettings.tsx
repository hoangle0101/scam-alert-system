...
import { User, ShieldCheck, Plug, Users, CreditCard, Edit3, KeyRound, Lock, RefreshCcw, Eye, Settings as SettingsIcon, CheckCircle, XCircle, Activity, ShieldAlert } from 'lucide-react';
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <Card className="bg-dark-800 border-dark-600 shadow-xl overflow-hidden relative group">
            ...
            <CardContent className="p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
              ...
              <Button variant="secondary" className="font-bold border-dark-600 hover:border-brand-500/50 bg-dark-900 py-3 px-8 text-xs tracking-widest uppercase shadow-lg">Export Cluster Logs</Button>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-8">
               <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-dark-700 border border-dark-600 flex items-center justify-center text-slate-500">
                      <Activity size={20} />
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-white uppercase tracking-tight">Latency Management</h4>
                      <p className="text-[10px] text-slate-500 font-mono">Current Cluster Ping: 14ms</p>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    {[1,2,3,4,5,6,7,8].map(i => (
                      <div key={i} className={`w-1 h-4 rounded-full ${i < 6 ? 'bg-brand-500' : 'bg-dark-700'}`}></div>
                    ))}
                  </div>
               </div>
            </CardContent>
